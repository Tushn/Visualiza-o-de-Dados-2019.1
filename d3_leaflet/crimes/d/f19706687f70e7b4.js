...
md`# Visualização de Crimes em Chicago`
)});
...
facts.dimension(d => d.longitude)
  ...
  main.variable(observer("typeGroup")).define("typeGroup", ["typeDim"], function(typeDim){return(
typeDim.group()
)});
  main.variable(observer("dayGroup")).define("dayGroup", ["dayDim"], function(dayDim){return(
dayDim.group().reduceCount()
)});
  main.variable(observer("scaleColors")).define("scaleColors", ["d3"], function(d3){return(
d3.scaleOrdinal().range(['#fdaf61', '#ca0020', '#0472b0']).domain(["BURGLARY", "HOMICIDE", "ROBBERY"])
)});
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","typeDim","d3","scaleColors","typeGroup","dataset","dayDim","dayGroup"], function(md,container,dc,typeDim,d3,scaleColors,typeGroup,dataset,dayDim,dayGroup)
{
  let view = md`${container('dc-table-graph', 'Crimes em Chicago')}`
...
L.layerGroup().addTo(map)
  ...
  main.variable(observer("circles")).define("circles", ["circlesLayer","dataset","L","scaleColors"], function(circlesLayer,dataset,L,scaleColors)
{
